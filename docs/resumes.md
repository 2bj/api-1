...
Принимает те же параметры что и [поиск по вакансиям](vacancies.md#search-params) и возвращает результаты в том же виде что и [поиск по вакансиям](vacancies.md#search-results)

Дополнительно, если резюме с идентификатором `resume_id` не существует, то в ответ вернется `404 Not Found`.
