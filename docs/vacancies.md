...
Выдача может отличаться при выборе [различных сайтов](hosts.md). Однако выбор
регионального сайта, например `hh.kz`, не сужает выборку до данного региона,
для ограничения выборки по региону используйте параметр [`area`](#field-area).
...
Принимает те же параметры что и [поиск по вакансиям](#search-params) и возвращает результаты в том же виде что и [поиск по вакансиям](#search-results)

Дополнительно, если вакансия с идентификатором `vacancy_id` не существует, то в ответ вернется `404 Not Found`.
